...
export class HierarchyTree
    ...
    public updateView(context: ComponentFramework.Context<IInputs>): void {
        ...
        for (const rid of currentIds) {
            ...
            // Capture JSON structure fields (priority over CSV columns)
            try {
                snap.strategyId = String(rec.getValue("strategyId") ?? "").trim();
            } catch { /* not available */ }
            try {
                snap.strategyName = String(rec.getValue("strategyName") ?? "").trim();
            } catch { /* not available */ }
            try {
                snap.strategyTypeKey = Number(rec.getValue("strategyTypeKey") ?? 0);
            } catch { /* not available */ }
            try {
                const levelsRaw = rec.getValue("levels");
                if (Array.isArray(levelsRaw)) {
                    snap.levels = levelsRaw.map(l => String(l ?? "").trim()).filter(l => l);
                }
            } catch { /* not available */ }
            try {
                snap.path = String(rec.getValue("path") ?? "").trim();
            } catch { /* not available */ }

            // Fallback: capture CSV columns for backward compatibility
            if (!snap.strategyId) {
                try {
                    snap.strategyId = String(rec.getValue("STRATEGY_ID") ?? "").trim();
                } catch { /* not available */ }
            }
            if (!snap.strategyName) {
                try {
                    snap.strategyName = String(rec.getValue("STRATEGY_NAME") ?? "").trim();
                } catch { /* not available */ }
            }
            if (!snap.strategyTypeKey) {
                try {
                    snap.strategyTypeKey = Number(rec.getValue("STRATEGY_TYPE_KEY") ?? 0);
                } catch { /* not available */ }
            }
