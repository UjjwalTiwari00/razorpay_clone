...
// ─────────────────────────────────────────────────────────────
// buildTree — updated to parse JSON levels array
// ─────────────────────────────────────────────────────────────
function buildTree(
  records: RecordSnapshot[],
  maxLevels: number,
  ...
  for (const rec of records) {
    ...
    if (strategyTypeKey > 0) {
      const typeKeyVal = rec.strategyTypeKey ?? Number(rec.values["STRATEGY_TYPE_KEY"] ?? 0);
      if (typeKeyVal !== strategyTypeKey) continue;
    }

    const strategyName = rec.strategyName ?? String(rec.values["STRATEGY_NAME"] ?? "").trim();
    const strategyId = rec.strategyId ?? String(rec.values["STRATEGY_ID"] ?? "").trim();

    // — Use pre-built path from JSON, or build from levels array, or fallback to CSV columns —
    let levels: string[] = [];
    let usePrebuiltPath = false;

    if (rec.path && rec.path.trim()) {
      // Use pre-built path from JSON
      usePrebuiltPath = true;
      levels = rec.path.split("|||").map(l => l.trim()).filter(l => l);
    } else if (rec.levels && Array.isArray(rec.levels)) {
      // Build from levels array
      levels = rec.levels;
    } else {
      // Fallback to CSV structure for backward compatibility
    ...
    if (filterName && levels.length > 0) {
      if (levels[0].toLowerCase() !== filterName.toLowerCase()) continue;
    }

    // — Build tree from levels array —
    let parentPath = "";
    let lastPath = "";
