...
interface TreeNode {
  ...
  strategyName?: string;
  strategyId?: string;
  // [FIX 1] Store array of strategies when multiple share the same path
  strategies?: { strategyName: string; strategyId: string }[];
}
...
function buildTree(
  ...
  // [FIX 1] Changed from single-value Map to array Map — stores ALL strategies per path
  ...
  for (const rec of records) {
    ...
    if (lastPath) {
      // [FIX 1] Append to array instead of overwriting — no data loss
  ...
  // [FIX 1] Attach metadata — handle single vs multiple strategies
  leafStrategiesMap.forEach((strategies, path) => {
    ...
    if (strategies.length === 1) {
      ...
      // Multiple strategies — store array + show count
      node.strategies   = strategies;
      node.strategyName = `${strategies.length} strategies`;
      node.strategyId   = strategies.map(s => s.strategyId).join(",");
    }
  });
...
function buildSearchIndex(roots: TreeNode[]): SearchEntry[] {
  ...
  function walk(node: TreeNode) {
    ...
    // [FIX 1] Include all individual strategy names/IDs in search haystack
    const texts = [
      displayLabel,
      node.strategyName ?? "",
      node.strategyId   ?? "",
      ...(node.strategies?.map(s => s.strategyName) ?? []),
      ...(node.strategies?.map(s => s.strategyId) ?? []),
      ...parts,
    ];
...
// [FIX 2] Removed findMatches — now done inline in useMemo with pre-built index
  ...
  ({
    ...
    return (
      <li style={{ listStyle: "none", margin: 0, padding: 0, position: "relative" }}>
        <div
          // [FIX 3] Removed unnecessary type cast
        ...
        {hasChildren && open && (
          <ul style={{ margin: 0, padding: 0 }}>
            {node.children.map((child, idx) => {
              ...
              // [FIX 4] Changed from depth >= 0 to depth > 0 to avoid parentLines[-1]
...
export const HierarchyTree: React.FC<Props> = ({
  ...
  // [FIX 2] Pre-build search index once per tree change — not per keystroke
  const searchIndex = useMemo(() => buildSearchIndex(tree), [tree]);

  // [FIX 2] Search matches using pre-built index
  const { matchPaths, expandPaths, matchList } = useMemo(() => {
    ...
    // Word-based search: every query word must appear somewhere in haystack
  ...
  useEffect(() => {
    ...
    const collectSelected = (node: TreeNode) => {
      // [FIX 1] Leaf node with no children
      ...
      // [FIX 1] Node with strategies array (multiple strategies at same path)
      if (node.strategies && checked.has(node.path)) {
        node.strategies.forEach((s) => {
          names.push(s.strategyName);
          ids.push(s.strategyId);
        });
        return;
      }
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: 11, color: "#605E5C", fontFamily: "Segoe UI, sans-serif" }}>
          {totalRows.toLocaleString()} strategies loaded
        </span>
      </div>
